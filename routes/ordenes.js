...
/* ─── Asignar pedido a ruta de repartidor ────────────────────────── */
router.get('/api/repartidores', async (req, res) => {
  try {
...
router.post('/api/rutas/asignar-pedido', async (req, res) => {
  ...
  try {
    ...
    await q(
      'INSERT INTO ruta_pedidos (ruta_id, pedido_id, geocerca_id, orden_planeado, estado) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE ruta_id = VALUES(ruta_id), geocerca_id = VALUES(geocerca_id)',
